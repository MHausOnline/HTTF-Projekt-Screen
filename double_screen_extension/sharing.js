...
let saved = document.documentElement.innerHTML
...
iframe.onload = () => {
	iframe.contentDocument.addEventListener("scroll", (event) => {
		...
		console.log(scrollX,scrollY,this);
	});
...
if(document.location.hash.includes("#right")){//or other logic
	document.documentElement.style.transform = "translateX(-100vw)"
	window.scrollTo(0,0);
}else{
	document.documentElement.style.transform = "translateX(0vw)"
...
function sendInteraction(element,event){
	if(role=="client"){
		socket.emit("sendAdmin",{"path":getElementPath(element),"event":event});
	}
}
